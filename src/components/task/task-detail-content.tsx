...
export function TaskDetailContent({ taskId, projectId, mode, onClose, onUpdated }: TaskDetailContentProps) {
 ...
 // Load project members (excluyendo rol Cliente — info interna)
 useEffect(() => {
 if (projectId) {
 api.get(`/projects/${projectId}/members?excludeRole=Cliente`).then((res) => {
 const list = Array.isArray(res.data) ? res.data : res.data?.data || [];
 ...
 if (isSheet) {
 // Sheet: navegar al subtask sin cerrar el sheet (cambiar taskId)
 // Simplificacion: en sheet se navega tambien a la URL del subtask
 router.push(`/projects/${projectId}/tasks/${subId}`);
 onClose?.();
 } else {
 router.push(`/projects/${projectId}/tasks/${subId}`);
 }
 ...
 if (loading) {
 return (
 <div className={isSheet ? 'p-6 space-y-4' : 'space-y-4'}>
 ...
 className="flex h-10 w-10 items-center justify-center rounded-full bg-card text-muted-foreground transition-colors hover:bg-muted"
 >
 ...
 <div className={`h-2.5 w-2.5 rounded-full ${STATUS_COLORS[task.status]}`} />
 <span className="text-[11px] text-muted-foreground uppercase tracking-wide">{STATUS_OPTIONS.find(s => s.value === task.status)?.label || task.status}</span>
 <Badge className={`text-[10px] ${priorityOpt?.color || 'bg-muted text-muted-foreground'}`}>{priorityOpt?.label || task.priority}</Badge>
 {task.type === 'SUPPORT' && (
 ...
 <div key={sub.id} className="flex items-center gap-3 rounded-xl border border-border p-3 cursor-pointer hover:bg-muted/50 transition-colors" onClick={() => handleSubtaskClick(sub.id)}>
 ...
 />
 <Button size="sm" variant="outline" className="h-7 px-2 text-xs" disabled={!hoursDirty || savingHours} onClick={saveHours}>
 {savingHours ? '...' : 'Guardar'}
 </Button>
 </div>
 </div>
 ...
 {/* Labels (con LabelSelector que permite editar) */}
 <div>
 <span className="flex items-center gap-1.5 text-muted-foreground text-xs mb-2"><Tag className="h-3 w-3"/> Etiquetas</span>
 <LabelSelector
 taskId={task.id}
 ...
 <button onClick={() => setShowRejections(!showRejections)} className="text-[10px] text-primary hover:underline">
 ...
 {/* Delete Task */}
 <div className={isSheet ? '' : 'rounded-xl border border-destructive/20 bg-card p-5'}>
 {!confirmDelete ? (
 <Button
 variant="outline"
 size="sm"
 ...
 <Button variant="outline" size="sm" className="flex-1" onClick={() => setConfirmDelete(false)}>Cancelar</Button>
 <Button
 variant="destructive"
 size="sm"
 className="flex-1"
 ...
 if (onClose) onClose();
 else router.push(`/projects/${projectId}/board`);
 } catch (err) {
