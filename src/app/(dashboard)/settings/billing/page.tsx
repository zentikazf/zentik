...
export default function BillingSettingsPage() {
  ...
  return (
    <div className="space-y-7">
      <div>
        <h1 className="text-[22px] font-semibold text-gray-800 dark:text-white">Facturación y Suscripción</h1>
        <p className="mt-1 text-sm text-gray-500 dark:text-gray-400">Gestiona tu plan, uso y historial de pagos</p>
      </div>
      ...
      <div className="rounded-[25px] bg-white p-6 dark:bg-gray-900">
        <div className="flex items-center justify-between">
          <div>
            ...
            <p className="mt-1 text-2xl font-bold text-gray-800 dark:text-white">{currentPlan}</p>
          </div>
          <div className="flex items-center gap-2">
            <Badge className="bg-green-50 text-green-600 dark:bg-green-950 dark:text-green-400">{subscription?.status || 'ACTIVE'}</Badge>
            {currentPlan !== 'FREE' && (
        ...
        {usage && (
          <div className="mt-5 grid grid-cols-3 gap-4">
            <div className="rounded-xl bg-blue-50 p-4 dark:bg-blue-950">
              ...
              <p className="mt-1 text-xl font-bold text-gray-800 dark:text-white">{usage.projects ?? 0}</p>
            </div>
            <div className="rounded-xl bg-purple-50 p-4 dark:bg-purple-950">
              ...
              <p className="mt-1 text-xl font-bold text-gray-800 dark:text-white">{usage.members ?? 0}</p>
            </div>
            <div className="rounded-xl bg-green-50 p-4 dark:bg-green-950">
              <p className="text-sm text-gray-500 dark:text-gray-400">Almacenamiento</p>
              <p className="mt-1 text-xl font-bold text-gray-800 dark:text-white">{usage.storageUsedGB ?? 0} GB</p>
            </div>
          </div>
        )}
      </div>
      ...
      {billingSummary && (
        <div className="rounded-[25px] bg-white p-6 dark:bg-gray-900">
          <h2 className="mb-5 text-lg font-semibold text-gray-800 dark:text-white">Resumen de Facturación</h2>
          <div className="grid gap-4 md:grid-cols-3">
            <div className="flex items-center gap-3">
              ...
              <div>
                ...
                <p className="text-xl font-bold text-gray-800 dark:text-white">{formatCurrency(billingSummary.totalInvoiced || 0)}</p>
              </div>
            </div>
            <div className="flex items-center gap-3">
              ...
              <div>
                ...
                <p className="text-xl font-bold text-gray-800 dark:text-white">{formatCurrency(billingSummary.totalPaid || 0)}</p>
              </div>
            </div>
            <div className="flex items-center gap-3">
              ...
              <div>
                ...
                <p className="text-xl font-bold text-gray-800 dark:text-white">{billingSummary.pendingInvoices ?? 0}</p>
              </div>
            </div>
          </div>
        </div>
      ...
      {/* Available Plans */}
      <div>
        <h2 className="mb-5 text-lg font-semibold text-gray-800 dark:text-white">Planes Disponibles</h2>
        <div className="grid gap-5 md:grid-cols-3">
          {displayPlans.map((plan: any) => (
            <div key={plan.id} className={`rounded-[25px] bg-white p-6 dark:bg-gray-900 ${currentPlan === plan.id ? 'ring-2 ring-blue-600' : ''}`}>
              <h3 className="text-lg font-bold text-gray-800 dark:text-white">{plan.name}</h3>
              <p className="mt-1 text-3xl font-bold text-gray-800 dark:text-white">{plan.price}<span className="text-sm font-normal text-gray-400">/mes por usuario</span></p>
              <ul className="mt-5 space-y-2.5">
                {(plan.features || []).map((f: string) => (
                  <li key={f} className="flex items-center gap-2 text-sm text-gray-600 dark:text-gray-300"><Check className="h-4 w-4 text-green-500" /> {f}</li>
                ))}
              </ul>
              <Button
      ...
      {subscriptionInvoices.length > 0 && (
        <div className="rounded-[25px] bg-white p-6 dark:bg-gray-900">
          <h2 className="mb-5 text-lg font-semibold text-gray-800 dark:text-white">Historial de Pagos</h2>
          <div className="space-y-2">
            {subscriptionInvoices.map((inv: any) => (
              <div key={inv.id} className="flex items-center justify-between rounded-xl border border-gray-100 p-4 dark:border-gray-800">
                <div>
                  <p className="text-[15px] font-medium text-gray-800 dark:text-white">{inv.description || inv.number || 'Pago de suscripción'}</p>
                  <p className="mt-0.5 text-[13px] text-gray-400">{formatDate(inv.createdAt || inv.date)}</p>
                </div>
                <div className="flex items-center gap-3">
                  <span className="font-medium text-gray-800 dark:text-white">{formatCurrency(inv.amount || inv.total || 0)}</span>
                  <Badge className={inv.status === 'PAID' ? 'bg-green-50 text-green-600 dark:bg-green-950 dark:text-green-400' : 'bg-yellow-50 text-yellow-600 dark:bg-yellow-950 dark:text-yellow-400'}>
