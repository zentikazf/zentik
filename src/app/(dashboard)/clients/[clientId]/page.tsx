...
export default function ClientDetailPage() {
 ...
 if (!orgId) return;
 try {
 const clientRes = await api.get<any>(`/organizations/${orgId}/clients/${clientId}`);
 ...
 const percentUsed = client.contractedHours > 0
 ? Math.min(Math.round((client.usedHours / client.contractedHours) * 100), 100)
 : 0;
 ...
 {/* Acceso a la vista de Tiempo (F2 #26) — horas contratadas, consumo y ledger */}
 <Link
 href={`/clients/${clientId}/tiempo`}
 className="flex items-center justify-between rounded-xl border border-border bg-card p-5 transition-colors hover:bg-muted/40"
 >
 <div className="flex items-center gap-3">
 <div className="flex h-11 w-11 items-center justify-center rounded-full bg-primary/10">
 ...
 <div>
 <p className="text-[15px] font-semibold text-card-foreground">Tiempo</p>
 <p className="text-sm text-muted-foreground">Horas contratadas, consumo y ledger</p>
 </div>
 </div>
 <ChevronRight className="h-5 w-5 text-muted-foreground"/>
 </Link>
