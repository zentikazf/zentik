...
export default function TareasPage() {
  ...
  if (loading) {
    return <Skeleton className="h-[calc(100vh-8rem)] w-full rounded-[25px]" />;
  }

  return (
    <div className="rounded-[25px] bg-white dark:bg-gray-900 h-[calc(100vh-8rem)] flex flex-col">
      {/* Header */}
      <div className="flex items-center justify-between px-6 py-4 border-b">
        <h1 className="text-lg font-semibold text-gray-800 dark:text-white">Tareas</h1>
            ...
            {filteredTasks.map((task) => (
              ...
              >
                <td className="px-6 py-3">
                  <p className="text-sm font-medium text-gray-800 dark:text-white truncate max-w-md">{task.title}</p>
                  {task._count?.subTasks > 0 && (
                    <span className="text-[10px] text-gray-400">{task._count.subTasks} subtarea(s)</span>
                  )}
                </td>
                ...
                <td className="px-3 py-3">
                  <div className="flex -space-x-1">
                    {(task.assignments || []).slice(0, 3).map((a: any) => (
                      <Avatar key={a.user?.id || a.id} className="h-6 w-6 border-2 border-white dark:border-gray-900">
