...
const EVENT_COLORS: Record<string, { bg: string; text: string }> = {
 task: { bg: 'bg-primary/10', text: 'text-primary' },
 sprint_start: { bg: 'bg-success/10', text: 'text-success' },
 sprint_end: { bg: 'bg-success/10', text: 'text-success' },
 meeting: { bg: 'bg-warning/10', text: 'text-warning' },
};
...
export default function ProjectCalendarPage() {
 ...
 <button onClick={prev} className="flex h-9 w-9 items-center justify-center rounded-full bg-card text-muted-foreground hover:bg-muted">
 <ChevronLeft className="h-4 w-4"/>
 </button>
 <span className="min-w-[160px] text-center text-[15px] font-semibold text-foreground">
 {MONTHS[month]} {year}
 </span>
 <button onClick={next} className="flex h-9 w-9 items-center justify-center rounded-full bg-card text-muted-foreground hover:bg-muted">
 <ChevronRight className="h-4 w-4"/>
 </button>
 <button onClick={todayBtn} className="ml-2 rounded-full bg-card px-4 py-1.5 text-sm font-medium text-muted-foreground hover:bg-muted">
 Hoy
 </button>
 </div>
 ...
 <span className="flex items-center gap-1.5"><span className="h-2.5 w-2.5 rounded-full bg-primary"/> Tareas</span>
 <span className="flex items-center gap-1.5"><span className="h-2.5 w-2.5 rounded-full bg-success"/> Sprints</span>
 <span className="flex items-center gap-1.5"><span className="h-2.5 w-2.5 rounded-full bg-warning/100"/> Reuniones</span>
 </div>

 <div className="flex gap-5">
 {/* Calendar Grid */}
 <div className="flex-1 rounded-2xl bg-card p-5">
 ...
 className={cn(
 'min-h-[80px] cursor-pointer rounded-lg border border-border p-1.5 transition-colors',
 day ? 'hover:bg-muted/50/50' : 'bg-muted/50',
 ...
 {dayEvts.length > 0 && (
 <span className="text-[9px] text-muted-foreground">{dayEvts.length}</span>
 )}
 </div>
 ...
 {dayEvts.length > 2 && (
 <span className="text-[9px] text-muted-foreground">+{dayEvts.length - 2} más</span>
 )}
 </div>
 </>
 )}
 ...
 {/* Day Detail Sidebar */}
 <div className="w-72 shrink-0 rounded-2xl bg-card p-5">
 ...
 className="flex h-7 w-7 items-center justify-center rounded-full text-muted-foreground hover:bg-muted hover:text-primary"
 >
 ...
 <p className="py-8 text-center text-xs text-muted-foreground">Sin eventos este día</p>
 )}
 ...
 <button onClick={() => openEditMeeting(rawMeeting)} className="text-muted-foreground hover:text-muted-foreground">
 <Pencil className="h-3 w-3"/>
 </button>
 <button onClick={() => handleDelete(rawMeeting.id)} className="text-muted-foreground hover:text-destructive">
 ...
 <p className="text-xs text-muted-foreground">El cliente recibirá una notificación en su dashboard</p>
 </div>
 </div>
 <Switch
